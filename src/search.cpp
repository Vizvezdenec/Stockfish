...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    // Set up the improving flag, which looks if static evaluation
    // is greater than the previous static evaluation at our turn (if we were
    // in check at our previous move we look at the move prior to it). 
    // The improving flag are used in various pruning heuristics.
    ...
    if (   !PvNode
        ...
        &&  eval >= beta
        &&  eval >= ss->staticEval
        &&  ss->staticEval >= beta - 20 * depth - improving * 22 + 204
        && !excludedMove

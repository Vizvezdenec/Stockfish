...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!PvNode && depth > 4 && !(ss-1)->forcenmp && !priorCapture && eval < ss->staticEval && is_ok((ss-1)->currentMove) && type_of((ss-1)->currentMove) == NORMAL && !(ss-1)->inCheck && !(ss-1)->nmattempt && abs(eval) < VALUE_KNOWN_WIN && eval <= alpha - 333)
    {

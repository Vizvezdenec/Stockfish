...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    else if (ss->ttHit)
    {
        ...
        ss->staticEval = eval = ss->staticEval + Value(thisThread->corrHistory[us][corr_structure(pos)] / 32);
...
moves_loop:  // When in check, search starts here
    ...
    {
        int adjust = int(bestValue - ss->staticEval) * depth / 8;
        thisThread->corrHistory[us][corr_structure(pos)] = std::clamp(thisThread->corrHistory[us][corr_structure(pos)] * 255 / 256 + adjust / 256,-CORR_HISTORY_LIMIT, CORR_HISTORY_LIMIT);
    }
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    else
    {
        if (ss->ttHit)
        {
            ...
            ss->staticEval = bestValue = ss->staticEval + Value(thisThread->corrHistory[us][corr_structure(pos)] / 32);

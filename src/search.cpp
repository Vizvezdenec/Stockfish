...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    noNmp = depth <= 5 && (pos.side_to_move() == WHITE ? pawn_attacks_bb<BLACK>(pos.pieces(BLACK, PAWN)) & (pos.pieces(WHITE, QUEEN, ROOK) | pos.pieces(WHITE, KNIGHT, BISHOP))
                                                       : pawn_attacks_bb<WHITE>(pos.pieces(WHITE, PAWN)) & (pos.pieces(BLACK, QUEEN, ROOK) | pos.pieces(BLACK, KNIGHT, BISHOP)));

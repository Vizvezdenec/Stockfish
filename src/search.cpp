                            ...
                            : (tte->bound() & BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            ...
            else if (!ttCapture)
            {
                ...
                thisThread->mainHistory[us][from_to(ttMove)][bool(threatened & from_sq(ttMove))][bool(threatened & to_sq(ttMove))] << penalty;
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          else
          {
              ...
              history += thisThread->mainHistory[us][from_to(move)][bool(threatened & from_sq(move))][bool(threatened & to_sq(move))];
      ...
      if (ss->ply < thisThread->rootDepth * 2)
      {
              ...
              &&  tte->depth() >= depth - 3)
          {
              ...
              // If the eval of ttMove is less than alpha and value, we reduce it (negative extension)
                  ...
                  extension = -1 - (value >= beta + 24 * depth);
          }
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          ss->statScore =  thisThread->mainHistory[us][from_to(move)][bool(threatened & from_sq(move))][bool(threatened & to_sq(move))]
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture(bestMove))
    {
        ...
        {
            thisThread->mainHistory[us][from_to(quietsSearched[i])][bool(threatened & from_sq(quietsSearched[i]))][bool(threatened & to_sq(quietsSearched[i]))] << -bonus2;
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, Bitboard threatened) {
    ...
    Color us = pos.side_to_move();
    Thread* thisThread = pos.this_thread();
    thisThread->mainHistory[us][from_to(move)][bool(threatened & from_sq(move))][bool(threatened & to_sq(move))] << bonus;

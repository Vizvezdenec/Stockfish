...
namespace {
  ...
  // Futility margin
  Value futility_margin(Depth d, bool stuff, bool improving) {
    return Value(140 * (d - improving) - 100 * stuff);
  }
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (   !ss->ttPv
        &&  depth < 9
        &&  eval - futility_margin(depth, cutNode && !ss->ttHit && depth >= 4, improving) - (ss-1)->statScore / 306 >= beta
        &&  eval >= beta

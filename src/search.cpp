...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (!excludedMove)
      ...
      Bitboard threatenedByMinor = pos.attacks_by<KNIGHT>(~us) | pos.attacks_by<BISHOP>(~us) | threatenedByPawn;
      // squares threatened by rooks, minors or pawns
      Bitboard threatenedByRook  = pos.attacks_by<ROOK>(~us) | threatenedByMinor;

      // pieces threatened by pieces of lesser material value
      Bitboard threatened =  (pos.pieces(us, QUEEN) & threatenedByRook)
        ...
        && (tte->bound() & (ttValue >= beta ? BOUND_LOWER : BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            ...
            else if (!ttCapture)
            {
                ...
                thisThread->mainHistory[us][bool(threatened & from_sq(ttMove))][from_to(ttMove)] << penalty;
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          else
          {
              ...
              history += 2 * thisThread->mainHistory[us][bool(threatened & from_sq(move))][from_to(move)];
              ...
              || (cutNode && (ss-1)->moveCount > 1)))
      {
          ...
          ss->statScore =  2 * thisThread->mainHistory[us][bool(threatened & from_sq(move))][from_to(move)]
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture(bestMove))
    {
        ...
        {
            thisThread->mainHistory[us][bool(threatened & from_sq(quietsSearched[i]))][from_to(quietsSearched[i])] << -bonus2;
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, Bitboard threatened) {
    ...
    Color us = pos.side_to_move();
    Thread* thisThread = pos.this_thread();
    thisThread->mainHistory[us][bool(threatened & from_sq(move))][from_to(move)] << bonus;

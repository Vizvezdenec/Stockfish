...
Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
        ...
        if (ttMove)
        {
            if (ttValue >= beta)
            {
                ...
                if (prevSq != SQ_NONE && (ss - 1)->moveCount <= 2 && !priorCapture)
                    update_continuation_histories(ss - 1, type_of((ss-1)->currentMove) != CASTLING ? pos.piece_on(prevSq) : us == WHITE ? B_KING : W_KING, prevSq,
                                                  -stat_malus(depth + 1));
            }
    ...
    if (is_ok((ss - 1)->currentMove) && !(ss - 1)->inCheck && !priorCapture)
    {
        ...
        if (type_of(pos.piece_on(prevSq)) != PAWN && type_of((ss - 1)->currentMove) != PROMOTION)
            thisThread->pawnHistory[pawn_structure(pos)][type_of((ss-1)->currentMove) != CASTLING ? pos.piece_on(prevSq) : us == WHITE ? B_KING : W_KING][prevSq] << bonus / 4;
    }
...
moves_loop:  // When in check, search starts here
    ...
    Move countermove =
      prevSq != SQ_NONE ? thisThread->counterMoves[type_of((ss-1)->currentMove) != CASTLING ? pos.piece_on(prevSq) : us == WHITE ? B_KING : W_KING][prevSq] : MOVE_NONE;
...
void update_all_stats(const Position& pos,
    ...
    if (prevSq != SQ_NONE
        ...
        && !pos.captured_piece())
        update_continuation_histories(ss - 1, type_of((ss-1)->currentMove) != CASTLING ? pos.piece_on(prevSq) : us == WHITE ? B_KING : W_KING, prevSq, -quietMoveMalus);
...
void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus) {
    ...
    if (is_ok((ss - 1)->currentMove))
    {
        ...
        thisThread->counterMoves[type_of((ss-1)->currentMove) != CASTLING ? pos.piece_on(prevSq) : us == WHITE ? B_KING : W_KING][prevSq] = move;
    }
}

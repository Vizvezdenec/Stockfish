...
namespace {
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
                        Move* quietsSearched, int quietCount, Move* capturesSearched, int captureCount, Depth depth, bool ttQuiet, Move ttMove);
...
moves_loop: // When in check, search starts from here
    ...
    bool ttQuiet = ttMove && !ttCapture;

    MovePicker mp(pos, ttMove, depth, &thisThread->mainHistory,
                                      &thisThread->lowPlyHistory,
                                      &captureHistory,
                                      contHist,
                                      &thisThread->doubleHistory,
                                      ttQuiet,
                                      countermove,
                                      ss->killers,
                                      ss->ply);
    ...
    */
    ...
    // If there is a move which produces search value greater than alpha we update stats of searched moves
        ...
        update_all_stats(pos, ss, bestMove, bestValue, beta, prevSq,
                         quietsSearched, quietCount, capturesSearched, captureCount, depth, ttQuiet, ttMove);
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture_or_promotion(bestMove))
    {
        ...
        if (ttQuiet && bestMove != ttMove)
            thisThread->doubleHistory[pos.moved_piece(ttMove)][to_sq(ttMove)][pos.moved_piece(bestMove)][to_sq(bestMove)] << bonus2;
        ...
        for (int i = 0; i < quietCount; ++i)
        {
            ...
            if (ttQuiet && quietsSearched[i] != ttMove && bestMove != ttMove)
                thisThread->doubleHistory[pos.moved_piece(ttMove)][to_sq(ttMove)][pos.moved_piece(quietsSearched[i])][to_sq(quietsSearched[i])] << -bonus2;
        }
    }
    else

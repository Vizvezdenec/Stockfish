...
Value to_corrected_static_eval(Value v, const Worker& w, const Position& pos, Stack* ss) {
    ...
    const auto cv =
      (5932 * pcv + 2994 * mcv + 3269 * macv + 5660 * micv + 6237 * (wnpcv + bnpcv) + cntcv * 5555 + 1333 * ftcv)
      / 131072;
    v += cv;
...
moves_loop:  // When in check, search starts here
        ...
        && !(!bestMove && bestValue >= ss->staticEval))
    {
        ...
        thisThread->nonPawnCorrectionHistory[BLACK][us][non_pawn_index<BLACK>(pos)]
          << bonus * 165 / 128;
        thisThread->fromToCorrectionHistory[~us][(ss-1)->currentMove.from_to()] << bonus * 117 / 128;

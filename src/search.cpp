...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
                            ...
                            : (tte->bound() & BOUND_UPPER)))
    {
        ...
        if (ttMove)
        {
            ...
            else if (!pos.capture_or_promotion(ttMove))
            {
                ...
                thisThread->ptHistory[us][pos.moved_piece(ttMove)][to_sq(ttMove)] << penalty / 16;
                thisThread->ptHistory[us][pos.moved_piece(ttMove)][from_sq(ttMove)] << -penalty / 16;
    ...
    if (is_ok((ss-1)->currentMove) && !(ss-1)->inCheck && !priorCapture)
    {
        ...
        thisThread->ptHistory[~us][pos.moved_piece((ss-1)->currentMove)][to_sq((ss-1)->currentMove)] << bonus / 16;
        thisThread->ptHistory[us][pos.moved_piece((ss-1)->currentMove)][from_sq((ss-1)->currentMove)] << -bonus / 16;
    }
...
moves_loop: // When in check, search starts from here
  ...
  void update_all_stats(const Position& pos, Stack* ss, Move bestMove, Value bestValue, Value beta, Square prevSq,
    ...
    if (!pos.capture_or_promotion(bestMove))
    {
        ...
        for (int i = 0; i < quietCount; ++i)
        {
            ...
            thisThread->ptHistory[us][pos.moved_piece(quietsSearched[i])][to_sq(quietsSearched[i])] << -bonus2 / 16;
            thisThread->ptHistory[us][pos.moved_piece(quietsSearched[i])][from_sq(quietsSearched[i])] << bonus2 / 16;
  ...
  void update_quiet_stats(const Position& pos, Stack* ss, Move move, int bonus, int depth) {
    ...
    thisThread->ptHistory[us][pos.moved_piece(move)][to_sq(move)] << bonus / 16;
    thisThread->ptHistory[us][pos.moved_piece(move)][from_sq(move)] << -bonus / 16;

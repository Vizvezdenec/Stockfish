...
namespace {
  ...
  Depth reduction(bool i, Depth d, int mn, Value delta, Value rootDelta, int ply) {
    int r = Reductions[d] * Reductions[mn];
    return (r + 1442 - int(delta) * 1024 / int(rootDelta) + 28 * ply) / 1024 + (!i && r > 916);
  }
...
moves_loop: // When in check, search starts here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          int lmrDepth = std::max(newDepth - reduction(improving, depth, moveCount, delta, thisThread->rootDelta, ss->ply), 0);
      ...
      {
          Depth r = reduction(improving, depth, moveCount, delta, thisThread->rootDelta, ss->ply);

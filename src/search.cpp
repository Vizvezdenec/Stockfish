...
namespace {
  ...
  Depth reduction(bool i, bool j, Depth d, int mn) {
    int r = Reductions[d] * Reductions[mn];
    return (r + 509) / 1024 + (!i && r > 894 * (1 + j));
  }
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
          ...
          int lmrDepth = std::max(newDepth - reduction(improving, sImproving, depth, moveCount), 0);
      ...
      {
          Depth r = reduction(improving, sImproving, depth, moveCount);

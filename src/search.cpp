...
namespace {
  ...
  template <NodeType nodeType>
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Value se, Depth depth = 0);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    if (depth <= 0)
        return qsearch<PvNode ? PV : NonPV>(pos, ss, alpha, beta, VALUE_NONE);
    ...
    {
        value = qsearch<NonPV>(pos, ss, alpha - 1, alpha, ss->staticEval);
        if (value < alpha)
            return value;
    }
    ...
    if (depth <= 0)
        return qsearch<PV>(pos, ss, alpha, beta, ss->staticEval);
             ...
             && ttValue < probCutBeta))
    {
        ...
        while ((move = mp.next_move()) != MOVE_NONE)
            if (move != excludedMove && pos.legal(move))
            {
                ...
                value = -qsearch<NonPV>(pos, ss+1, -probCutBeta, -probCutBeta+1, VALUE_NONE);
...
moves_loop: // When in check, search starts here
  ...
  // (~155 Elo)
  template <NodeType nodeType>
  Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Value se, Depth depth) {
    ...
    while ((move = mp.next_move()) != MOVE_NONE)
    {
        ...
        pos.do_move(move, st, givesCheck);
        value = -qsearch<nodeType>(pos, ss+1, -beta, -alpha, VALUE_NONE, depth - 1);
        pos.undo_move(move);

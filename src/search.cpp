...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
      ...
      bool givesDiscoCheck = false;
      if (givesCheck)
          givesDiscoCheck = pos.gives_disco_check(move);
          ...
          && bestValue > VALUE_TB_LOSS_IN_MAX_PLY)
      {
              ...
              && !givesCheck)
          {
              ...
              if (!givesDiscoCheck && !pos.see_ge(move, Value(-(30 - std::min(lmrDepth, 18)) * lmrDepth * lmrDepth)))
                  continue;
          }
          else
          {
              ...
              // SEE based pruning
              if (!givesDiscoCheck && !pos.see_ge(move, Value(-213) * depth)) // (~25 Elo)
                  continue;
          }
      }

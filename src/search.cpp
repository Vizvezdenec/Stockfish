...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool probcutLoop);
...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode, bool probcutLoop) {
        ...
        && !probcutLoop)
    {
        ...
        while (   (move = mp.next_move()) != MOVE_NONE
            ...
            if (move != excludedMove && pos.legal(move))
            {
                ...
                if (value >= raisedBeta)
                    value = -search<NonPV>(pos, ss+1, -raisedBeta, -raisedBeta+1, depth - 4, !cutNode, true);
    ...
    if (depth >= 7 && !ttMove)
    {
        search<NT>(pos, ss, alpha, beta, depth - 7, cutNode, false);
...
moves_loop: // When in check, search starts from here
    ...
    while ((move = mp.next_move(moveCountPruning)) != MOVE_NONE)
    {
          ...
          &&  pos.legal(move))
      {
          ...
          value = search<NonPV>(pos, ss, singularBeta - 1, singularBeta, singularDepth, cutNode, false);

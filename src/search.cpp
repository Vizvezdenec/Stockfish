...
namespace {
  ...
  Value search(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth, bool cutNode) {
    ...
    {
        Value raisedBeta = std::min(beta + 191 - 46 * improving + 300 * (depth < 5 * ONE_PLY), VALUE_INFINITE);
        ...
        while (  (move = mp.next_move()) != MOVE_NONE
            ...
            if (move != excludedMove && pos.legal(move))
            {
                ...
                if (value >= raisedBeta)
                    value = -search<NonPV>(pos, ss+1, -raisedBeta, -raisedBeta+1, std::max(depth - 4 * ONE_PLY, ONE_PLY), !cutNode);

...
moves_loop:  // When in check, search starts here
    ...
    if (!ss->inCheck 
        ...
        thisThread->corrHistory[us][corr_structure(pos)] << std::clamp(int(bestValue - ss->staticEval) * (8 + depth) / 8, -CORR_HISTORY_LIMIT, CORR_HISTORY_LIMIT);
...
Value qsearch(Position& pos, Stack* ss, Value alpha, Value beta, Depth depth) {
    ...
    if (!ss->inCheck 
        && !(bestValue >= beta && bestValue <= ss->staticEval)
        && !(bestValue <= alpha && bestValue >= ss->staticEval))
        thisThread->corrHistory[us][corr_structure(pos)] << std::clamp(int(bestValue - ss->staticEval) * std::max(8 + depth, 0) / 8, -CORR_HISTORY_LIMIT, CORR_HISTORY_LIMIT);

...
MovePicker::MovePicker(const Position& p, Move ttm, int th, const CapturePieceToHistory* cph, bool gc) :
    pos(p),
    captureHistory(cph),
    ttMove(ttm),
    threshold(th),
    ...
    stage = PROBCUT_TT
          + !(ttm && (gc || (pos.capture_stage(ttm) && pos.pseudo_legal(ttm) && pos.see_ge(ttm, threshold))));
}

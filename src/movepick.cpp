  ...
  for (auto& m : *this)
      if constexpr (Type == CAPTURES)
          ...
          m.value =  2 * (*mainHistory)[pos.side_to_move()][from_to(m)]
                   ...
                   + (type_of(pos.moved_piece(m)) == PAWN && pos.pawn_passed(pos.side_to_move(), to_sq(m))) * 2000 * 
                     std::max(relative_rank(pos.side_to_move(), to_sq(m)) - 3, 0) * std::max(relative_rank(pos.side_to_move(), to_sq(m)) - 3, 0);
      else // Type == EVASIONS
      {

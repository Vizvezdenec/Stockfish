...
void MovePicker::score() {
  ...
  for (auto& m : *this)
      if (Type == CAPTURES)
          m.value =  int(PieceValue[MG][pos.piece_on(to_sq(m))]) * 6 + 1000 * pos.gives_check(m)
                   + (*captureHistory)[pos.moved_piece(m)][to_sq(m)][type_of(pos.piece_on(to_sq(m)))] * 3 / (3 + pos.gives_check(m));

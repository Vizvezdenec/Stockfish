...
Value Eval::evaluate(const Position& pos, int* complexity) {
  ...
  bool useClassical =    (pos.this_thread()->depth > 9 || pos.count<ALL_PIECES>() > 7)
                      && abs(psq) * 5 > (872 + pos.non_pawn_material() / 67) * (10 + pos.rule50_count());
  ...
  {
       int nnueComplexity;
       int scale = 1076 + 107 * pos.non_pawn_material() / 5120;
       Value optimism = pos.this_thread()->optimism[stm];

       Value nnue = NNUE::evaluate(pos, true, &nnueComplexity);
       // Blend nnue complexity with (semi)classical complexity
       nnueComplexity = (99 * nnueComplexity + 126 * abs(nnue - psq)) / 256;
       if (complexity) // Return hybrid NNUE complexity to caller
           *complexity = nnueComplexity;

       optimism = optimism * (264 + nnueComplexity) / 256;
       v = (nnue * scale + optimism * (scale - 742)) / 1024;
  }

  // Damp down the evaluation linearly when shuffling
  v = v * (193 - pos.rule50_count()) / 212;

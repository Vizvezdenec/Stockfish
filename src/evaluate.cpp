...
#define S(mg, eg) make_score(mg, eg)
  ...
  constexpr Score OutpostNoMinors    = S( 24,  8); 
  ...
  Score Evaluation<T>::pieces() {
    ...
    for (Square s = *pl; s != SQ_NONE; s = *++pl)
    {
        ...
        if (Pt == BISHOP || Pt == KNIGHT)
        {
            ...
            bool enemyMinors = pos.count<KNIGHT>(Them) + pos.count<BISHOP>(Them);
            if (bb & s)
                score += (enemyMinors ? Outpost : OutpostNoMinors) * (Pt == KNIGHT ? 4 : 2);

            else if (bb & b & ~pos.pieces(Us))
                score += (enemyMinors ? Outpost : OutpostNoMinors) * (Pt == KNIGHT ? 2 : 1);

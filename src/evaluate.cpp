...
#define S(mg, eg) make_score(mg, eg)
  ...
  constexpr Score KnightOnQueen      = S( 32, 24);
  ...
  constexpr Score SliderOnQueen      = S(108, 36);
...
#undef S
  ...
  Score Evaluation<T>::threats() const {
    ...
    while (bb)
    {
        ...
        score += KnightOnQueen * popcount(b & safe) / (1 + pos.count<QUEEN>(Them));

        b =  (attackedBy[Us][BISHOP] & pos.attacks_from<BISHOP>(s))
           | (attackedBy[Us][ROOK  ] & pos.attacks_from<ROOK  >(s));

        score += SliderOnQueen * popcount(b & safe & attackedBy2[Us]) / (1 + pos.count<QUEEN>(Them));
    }
